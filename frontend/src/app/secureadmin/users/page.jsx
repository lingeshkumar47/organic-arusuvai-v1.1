...
export default function UserManagement() {
  ...
  async function deleteUser(id) {
    if (confirm("Are you sure? This will remove the user profile!")) {
      ...
      showToast("User removed securely.");
      fetchUsers();
    }
  }
  ...
  async function addLocalOrder() {
    ...
    const { data: order, error: oError } = await supabase.from('orders').insert({
      ...
      created_at: new Date(orderData.date).toISOString()
    }).select().single();
    ...
    setOrderData({ productId: '', variant: '', quantity: 1, price: 0, paymentStatus: 'paid', date: new Date().toISOString().split('T')[0] });
  }
  ...
  return (
    <div className="pb-10">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 animate-slide-up">
             ...
             <div className="space-y-4">
                ...
                <div>
                   <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Date of Order</label>
                   <input type="date" value={orderData.date} onChange={e => setOrderData({...orderData, date: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-bold block" />
                </div>
      ...
      {/* TOAST NOTIFICATION */}
      {toast && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] bg-gray-900 text-white font-bold text-sm px-6 py-3 rounded-full flex items-center gap-3 shadow-float animate-slide-up">
